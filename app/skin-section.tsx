...
import {sectionProfile,SectionRelaxation,deformSection} from '@/lib/skin-section-model';
...
export default function SkinSection({region,params,sceneRef,onClose,onParameters}:{region:SkinRegion|null;params:Parameters;sceneRef:MutableRefObject<AnatomyScene|null>;onClose:()=>void;onParameters:(patch:Partial<Parameters>)=>void}){
 return <Dialog open={!!region} onOpenChange={open=>{if(!open)onClose()}}><DialogContent className="skin-section-dialog"><DialogTitle>{region?.label} <span className="section-chip">3D 조직 단면</span></DialogTitle><DialogDescription>굴곡진 피부층, 결합조직과 지방 소엽, 혈관벽을 회전·확대하여 관찰합니다.</DialogDescription>{region&&<SectionViewer key={region.id+region.position.join(',')} region={region} params={params} sceneRef={sceneRef} onParameters={onParameters}/>}</DialogContent></Dialog>;
}
function SectionViewer({region,params,sceneRef,onParameters}:{region:SkinRegion;params:Parameters;sceneRef:MutableRefObject<AnatomyScene|null>;onParameters:(patch:Partial<Parameters>)=>void}){
 ...
 useEffect(()=>{
  ...
  const draw=(now:number)=>{
   ...
   if(now-stats>160&&hud.current){stats=now;const centre=deformSection(0,0,0,profile,state,1)[1],side=deformSection(Math.max(1.7,profile.arteryDepth*.9),0,0,profile,state,1)[1];hud.current.textContent=`중앙 변위 ${centre.toFixed(3)} mm · 주변 변위 ${side.toFixed(3)} mm · 내강 반경 ${(profile.radius+state.distension).toFixed(3)} mm${c.light?` · ${c.mode==='reflection'?'상부':'하부'} 출사 광량 ${(energy[c.mode==='reflection'?'reflection':'transmission']*100).toFixed(1)}%`:''}`;}
  };
 ...
 return <>
  ...
  <div className="section-tissue-legend"><span><i style={{background:'#e6c3a7'}}/>각질·표피</span><span><i style={{background:'#d59b9a'}}/>유두·망상 진피</span><span><i style={{background:'#d2b581'}}/>지방 소엽·섬유 격막</span><span><i style={{background:'#ddc8b6'}}/>동맥벽 3층</span><span><i style={{background:'#648797'}}/>정맥·모세혈관</span></div>
  <div ref={hud} className="section-hud" aria-live="off"/>
  ...
  <p className="section-note"><Layers size={14}/><span>해부학 구조를 참고한 3D 조직 모형입니다. 미세조직의 일부 크기는 가독성을 위해 강조했습니다. 층별 두께와 점성은 예시값이며, 조직 변형은 주변 단면적 보존과 점성 이완을 근사합니다. 개인별 물성으로 보정한 3D 유한요소 해석은 아닙니다. 광 경로는 출사 경향을 보여주며 SpO₂ 역산값을 제공하지 않습니다. <a href="https://www.ncbi.nlm.nih.gov/books/NBK535346/" target="_blank" rel="noreferrer">조직 구조</a> · <a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC2584606/" target="_blank" rel="noreferrer">점탄성 참고</a></span></p>
 </>;
}
