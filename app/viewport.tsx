...
export default function Viewport({comfortMode,params,layers,selectedSites,skinInspection,onSkin,sceneRef,onStats,onPick,onTime,onSite}:{comfortMode:boolean;params:Parameters;layers:Layers;selectedSites:Site[];skinInspection:boolean;onSkin:(region:SkinRegion)=>void;sceneRef:MutableRefObject<AnatomyScene|null>;onStats:(fps:number,triangles:number)=>void;onPick:(s:string)=>void;onTime:(t:number)=>void;onSite:(s:Site)=>void}){
 ...
 useEffect(()=>{let scene:AnatomyScene;let active=true;try{scene=new AnatomyScene(host.current!,params,layers,{stats:(...a)=>callbacks.current.stats(...a),pick:s=>callbacks.current.pick(s),time:t=>callbacks.current.time(t),site:s=>callbacks.current.site(s),skin:r=>callbacks.current.skin(r)});sceneRef.current=scene;scene.setComfortMode(comfortMode);scene.setSensors(selectedSites);scene.setSkinInspection(skinInspection);scene.load(n=>{if(active)setProgress(n)}).catch(()=>{if(active)setError('해부학 모델을 불러오지 못했습니다. 연결을 확인하고 새로고침해 주세요.')});}catch{setError('WebGL 2를 사용할 수 없습니다. 브라우저의 하드웨어 가속을 켜 주세요.');}return()=>{active=false;scene?.dispose();sceneRef.current=null;};},[]);
