...
영문 레이아웃의 칩·카드·그래프 범례/설명·피부 단면 라벨 크기를 보완했다. 전신 움직임에는 CMU 기반 오른손 인사(141_16), Charleston 춤(103_03)을 추가했다. 출처·생성 절차는 public/motions/ATTRIBUTION.md와 docs/motion/clinical-motions.md에 기록했다. 최신 검사: 109개 테스트, TypeScript·사이트 빌드, 언어별 페이지/다운로드 106개와 새 모션 메뉴 HTTP 확인 통과. 브라우저 연결이 없어 화면 시각 검증은 미수행이며 골격 자세 도식과 수치 검사를 수행했다.


한국어·영어 전환을 구현했다. 상단 스위치, 전신·손목 UI, 캔버스 라벨, 매뉴얼·연구 문서 15쌍, UTF-8 Markdown 다운로드를 지원한다. 언어는 쿠키에 저장하며 전환 시 시뮬레이션이 재시작된다. 구현과 갱신 절차는 [다국어 유지보수](localization.md)를 따른다.

검사 결과: 회귀 테스트 103개 통과, TypeScript·빌드·문서 검사 통과, 두 언어 페이지와 다운로드 106개 HTTP 응답 검증. 브라우저 시각 검사나 실제 Windows 재검증은 수행하지 않았다. 현재 사이트 접근 범위는 사용자가 설정한 공개이며 그대로 보존한다. 이 변경을 커밋·푸시하고 배포 및 데모 재패키징한다. 정확한 최종 상태는 Git HEAD, Sites 배포 상태와 demo manifest에서 확인한다.
