...
오른손 인사와 춤의 혈관·근육 연결을 해부학 메시 기준으로 수정했다. 춤은 CMU 90_28 브레이크댄스(338–700 프레임)로 교체하고 바닥 지지를 손·몸통까지 포함했다. 683개 조직의 큰 체적 팽창을 제한하는 GPU 보정표를 추가했다. 침대 옆눕기 이후 양팔은 펴진 상태를 유지한다. 심장 아래 혈관으로 과도하게 전파되던 심근 변형은 짧은 혈관 뿌리에만 감쇠해 적용한다. [모션 기록](../motion/clinical-motions.md)을 참고한다.
