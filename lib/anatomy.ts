...
import {HumanRig,bindGeometry,rigidBone,pelvicOrgan,rigShader,weightsAt,surfaceTissueWeight} from './rig';
import {respiratoryPart,isRespiratoryPart} from './respiratory.js';
import {registerSkinGeometry} from './skin-registration.js';
import {bindCardiacMotion,cardiacShader} from './cardiac';
...
export class AnatomyScene{
 ...
 const gltf=await loader.loadAsync(layer==='skin'?'/models/skin-fitted-web.glb?envelope=2':`/models/${layer}-web.glb${layer==='adipose'||layer==='dermis'?'?tissue=3':layer==='visceral'?'?lungs=4':''}`);if(this.disposed){gltf.scene.traverse(o=>{if(o instanceof THREE.Mesh)o.geometry.dispose()});return;}
 ...
 this.applyDeformation(mat,layer);if(layer!=='skin'||/Male[_ ]skin/i.test(obj.name))applySkinTissue(mat,layer);
 ...
 const part=layer==='visceral'&&isHepatic(name)?'hepatic':layer==='visceral'&&pelvicOrgan(name)?'pelvic':layer==='muscular'?muscleTissue(name):layer==='visceral'&&respiratoryPart(name)?respiratoryPart(name)!:layer==='cardiovascular'&&/atrium|ventricle|myocard|epicard/i.test(name)?'heart':layer==='cardiovascular'&&isArtery(name)?'artery':'body';
 ...
 this.applyDeformation(mat,part,layer==='skeleton'||part==='pelvic'||part==='hepatic'||isRespiratoryPart(part),layer==='cardiovascular');
