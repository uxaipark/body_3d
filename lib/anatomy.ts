...
export class AnatomyScene{
 ...
 const gltf=await loader.loadAsync(layer==='skin'?'/models/skin-atlas-web.glb?native=1':`/models/${layer}-web.glb${layer==='adipose'||layer==='dermis'?'?tissue=3':layer==='visceral'?'?lungs=4':''}`);if(this.disposed){gltf.scene.traverse(o=>{if(o instanceof THREE.Mesh)o.geometry.dispose()});return;}
 ...
 this.applyDeformation(mat,layer);if(layer!=='skin'||/Atlas[_ ]Skin/i.test(obj.name))applySkinTissue(mat,layer);
 ...
 gltf.scene.traverse(obj=>{if(!(obj instanceof THREE.Mesh))return;
 const name=obj.name.replace(/_/g,' ');if(layer==='visceral'&&respiratoryPart(name)==='pleura')return;if(/systemg\d|organsg\d/i.test(name))return;
 const source=Array.isArray(obj.material)?obj.material[0]:obj.material;
 ...
 const mesh=new THREE.Mesh(geometry,mat);mesh.userData={layer,organSurface:part==='heart'||part==='lung',ranges:batch.ranges,opacityScale:part==='fascia'?.13:part==='pleura'?.18:1};mesh.renderOrder=part==='fascia'?6:layer==='muscular'?4:layer==='skeleton'?3:part==='pleura'?1:0;this.meshes.push(mesh);group.add(mesh);}
