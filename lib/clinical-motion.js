...
export function taskState(motion,elapsed){
 ...
 if(motion==='lie'){
  ...
  return {...base,seat:prep,arm:prep,recline:sideLower,legLift,sideLower,extend,roll,effort,activity:effort*.7,repetitions:t>=13?1:0,complete:t>=13,stage:t<2?'침대 왼쪽 중간에 걸터앉기':t<3?'앉아서 지지 준비':t<7.2?'머리맡으로 옆눕기 · 다리 올리기':t<9.5?'다리 펴기 · 등으로 돌아눕기':t<13?'등으로 돌아누워 내려놓기':'누운 자세 · 안정 호흡'};
 }
 if(motion==='grip'){
