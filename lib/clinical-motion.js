...
export const motionOptions=[['rest','정지'],['walk','걷기'],['run','달리기'],['stand','일어서기'],['sitStand','앉았다 일어서기'],['grip','손 쥐기']];
export const ease=(a,b,t)=>{const u=Math.max(0,Math.min(1,(t-a)/(b-a)));return u*u*u*(u*(u*6-15)+10)};
export const isClinicalMotion=motion=>['stand','sitStand','grip'].includes(motion);
export function taskState(motion,elapsed){
 const t=Math.max(0,elapsed),base={seat:0,lean:0,grip:0,arm:0,effort:0,activity:0,stage:'',repetitions:0,complete:false};
