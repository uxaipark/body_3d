...
export function applySkinTissue(mat:THREE.MeshStandardMaterial,layer:'dermis'|'adipose'){
 ...
 vec3 tissuePhase=vSkinTissue*${layer==='adipose'?'620.0':'3500.0'};
 ...
 diffuseColor.rgb*= ${layer==='adipose'?'(.78+.30*detail)':'(.93+.10*detail)'};
 float tissueHeight=detail*${layer==='adipose'?'0.00023':'0.00002'};
 `);
 ...
 mat.customProgramCacheKey=()=>`${cache}-${layer}-surface-v1`;
}
