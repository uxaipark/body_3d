...
export const sleepDefaults={rr:14,hr:62,tidal:.5,compliance:.2,resistance:2,area:180,gap:2,permittivity:6,noise:.015,scenario:'normal'};
...
export class SleepGenerator{
 ...
 step(dt,settings=sleepDefaults,pose={roll:0,motion:0}){
  ...
  const excursion=.008*(obstruction?effort*p.tidal/.5:this.volume/.5);
  ...
  const speed=(pose.roll-this.lastRoll)/dt;this.lastRoll=pose.roll;
  ...
  const cap=[0,1,2].map(i=>{const strain=excursion*(8+i*.5),area=p.area*1e-6*(1+strain),gap=p.gap*1e-3*(1-.045*excursion/.008)+motionNoise*.00012;return 9+i*.8+8.8541878128e-12*p.permittivity*area/Math.max(.0004,gap)*1e12+this.random()*p.noise+motionNoise*(i+1)*.2;});
  this.cardiac+=dt*p.hr/60*(1+.035*Math.sin(this.phase));const phase=this.cardiac%1;
  const g=(c,w,a)=>a*Math.exp(-.5*((phase-c)/w)**2);
  const baseline=.13*(effort-.5)+.8*motionNoise;
  const ecg=(g(.16,.035,.12)-g(.36,.012,.15)+g(.4,.009,1.1)-g(.435,.014,.23)+g(.68,.065,.29))*(1+.08*(effort-.5))+baseline+this.random()*.006;
  const angle=pose.roll+.035*effort;
  const acc=[9.80665*Math.sin(angle)+motionNoise*.6,acceleration+this.random()*.005,9.80665*Math.cos(angle)+motionNoise*.3];
  ...
  return {t,pose,cardiac:this.cardiac,cap,ecg,acc,breathAudio,snore,camera:motion,radar:[excursion*1000+motionNoise*8,pose.roll*100,0,0],airflow:flow,truth:{volume:this.volume,effort,pressure,flow,event,obstruction,motion,excursion,rr:p.rr}};
 }
}
...
export class SleepAnalysis{
 ...
 ingest(input){
  ...
  for(let i=0;i<p.samples.length;i++){
   ...
   if(p.kind==='ecg'){
    channel.base??=s;channel.base+=Math.min(1,dt/.7)*(s-channel.base);v=channel.base;
    // R-peak HR from the acquired ECG, never from the generator's HR parameter.
    if(s-channel.base>.55&&t-(channel.beat??-Infinity)>.4){if(channel.beat!==undefined){const interval=t-channel.beat;if(interval>.4&&interval<2){this.rrIntervals.push(interval);if(this.rrIntervals.length>8)this.rrIntervals.shift();}}channel.beat=t;this.lastBeat=t;}
   }
   channel.raw.push({t,value:s});
   if(!channel.series.length||t-channel.series.at(-1).t>=.035)channel.series.push({t,v});channel.last=t;
  }
  ...
  channel.received=Date.now();return true;
 }
 analyze(now,enabled={cap:true,ecg:false,acc:false,breathAudio:false,radar:false}){
  ...
  for(const [id,c]of this.channels){if(!enabled[c.kind]||now-c.last>.6)continue;const clean=c.series.filter(p=>p.t>validAfter),recent=clean.filter(p=>p.t>now-6);if(recent.length){const mean=recent.reduce((a,p)=>a+p.v,0)/recent.length,rms=Math.sqrt(recent.reduce((a,p)=>a+(p.v-mean)**2,0)/recent.length);if(c.kind==='cap'&&rms<.02||c.kind==='acc'&&rms<.01)continue;}const estimate=respirationEstimate(clean);if(estimate&&estimate.quality>.25)candidates.push({id,kind:c.kind,...estimate});}
  ...
  const hr=now-this.lastBeat<3&&this.rrIntervals.length?60/(this.rrIntervals.reduce((a,b)=>a+b,0)/this.rrIntervals.length):null;
