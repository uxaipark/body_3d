...
상단 **성능 모드**에서 자동·저사양·균형·고화질을 선택합니다. 전신 트윈과 선택값을 공유하며 브라우저에 저장합니다. 저사양은 3D 최대 30FPS와 낮은 해상도, 균형은 전신 경량 모델과 확대 영역 정밀 모델, 고화질은 보이는 정밀 모델을 사용합니다. 자동은 기기 사양과 재생 중 프레임 속도로 판단합니다. CAP·ECG·ACC 생성 및 소켓 입력·융합 분석 주기는 성능 모드에 따라 바뀌지 않습니다.
