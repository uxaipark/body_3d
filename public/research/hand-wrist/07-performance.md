...
전신·손목 보행은 동일한 캡처와 보행 주기(1.1166667초)를 사용한다. 패치는 피부 둘레의 호 길이를 따라 감기며 `,`/`.`로 2.5°씩 회전한다. 회전한 전극의 좌표는 신호 생성 모델에도 전달된다. 중앙 빈 공간의 좌우 드래그는 화면 Z축 회전이다. 손목 표시 패널 높이는 430→860 px, 좁은 화면은 320→640 px로 바꿨다.
