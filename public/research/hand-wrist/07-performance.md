...
수정 후 로컬 Node.js 단일 실행 참고값: 손목 GLB 파싱·초기화 약 132 ms, 120회 평균 손목 조직 CPU 갱신 약 0.42 ms/회, 9채널 기본 신호 모델은 16 ms 시뮬레이션 구간당 CPU 약 1.07 ms. 이는 GPU·렌더링·브라우저 작업·WASM 분석을 제외한 값이며 기기와 설정에 따라 달라진다. 화면 렌더링 병목이 확정됐다는 근거로 사용하지 않는다.
...
전신·손목 보행은 동일한 캡처와 보행 주기(1.1166667초)를 사용한다. 패치는 피부 둘레의 호 길이를 따라 감기며 `,`/`.`로 2.5°씩 회전한다. 회전한 전극의 좌표는 신호 생성 모델에도 전달된다. 중앙 빈 공간의 좌우 드래그는 화면 Y축 회전 · 옆면 보기이다. 손목 표시 패널 높이는 430→860 px, 좁은 화면은 320→640 px로 바꿨다.

요골동맥은 아틀라스에서 추출한 중심선을 160개 길이 구간·40개 원주 구간으로 매끄럽게 렌더링한다. 반경 1.1 mm는 기존 조직 모델의 대표값이며, 원본 혈관의 개인별 직경을 재구성한 것은 아니다. 좌우·깊이 조절량은 신호 생성, 3D 혈관·주변 조직 및 절단면에 연결된다. 압력에 따른 기존 인과적 반경 변화가 혈관의 팽창·수축을 구동한다.
