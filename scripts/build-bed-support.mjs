...
import {HumanRig,surfaceFootSupport} from '../lib/rig.ts';import {aboveBed,worldToBed} from '../lib/bed.js';
...
for(let time=0;time<=21;time+=.25)for(const lift of [0,.07,.16]){
 rig.poseBed(time);rig.bones[0].position.y+=lift;rig.bones[0].updateMatrixWorld(true);rig.updatePalette();const cells=new Map();
 for(let i=0;i<candidates.length;i++){const c=candidates[i],v=rig.transform(c.point,c.w);const [x,z]=worldToBed(v.x,v.z,rig.bedAnchor);if(!aboveBed(x,z))continue;const key=[Math.floor(x/.10),Math.floor(z/.10)].join(',');if(!cells.has(key)||cells.get(key).y>v.y)cells.set(key,{i,y:v.y});}
