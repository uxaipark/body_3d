...
test('bed transfer preserves limb lengths, faces upward at rest, holds then returns to sitting',()=>{
 ...
 rig.poseBed(8);const hold=rig.bones.map(b=>b.matrixWorld.clone());const normal=new T.Vector3(0,0,1).applyQuaternion(rig.bone('head').getWorldQuaternion(new T.Quaternion()));assert.ok(normal.y>.99);
 rig.poseBed(16);rig.bones.forEach((b,i)=>b.matrixWorld.elements.forEach((v,j)=>near(v,hold[i].elements[j])));
 assert.equal(taskState('lie',22).complete,true);assert.equal(taskState('lie',22).repetitions,1);
 rig.poseBed(22);assert.ok(new T.Vector3(0,1,0).applyQuaternion(rig.bone('head').getWorldQuaternion(new T.Quaternion())).y>.99);
});
test('full native skin clears the actual mattress and remains supported through the complete transfer',async()=>{
 ...
 rig.poseBed(10);const contacts={head:Infinity,back:Infinity,pelvis:Infinity,heels:Infinity};
...
test('bed task pauses and restarts on the shared clock and CSV records its phase',()=>{
 ...
 const rows=csv({...defaults,motion:'lie',motionStartedAt:10},1,10,18).split('\n');assert.ok(rows[1].includes('lie,8.00000,누워서 안정 호흡'));
});
