...
for(const node of doc.getRoot().listNodes())if(node.getMesh())for(const primitive of node.getMesh().listPrimitives()){
 ...
 for(let i=0;i<p.count;i+=Math.max(1,Math.floor(p.count/20))){const point=new T.Vector3().fromBufferAttribute(p,i);const face=guard.boundary.nearest(point);samples.push({name:node.getName(),original:new T.Vector3().fromBufferAttribute(original,i),point,face:face.ids,anchor:guard.anchor(point,face.ids),w:{indices:[0,1,2,3].map(j=>ix.getComponent(i,j)),weights:[0,1,2,3].map(j=>w.getComponent(i,j))}});}
 nerve.dispose();
}
